    ...
    <head>
        ...
        <script type="text/javascript">
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if(routeDataStatus === '0') {
                                        ...
                                        // 노선전체를 현시할 지도영역 구하기
                                        maxX = Number(getMinMaxXY('max', 'x'))
                                        maxY = Number(getMinMaxXY('max', 'y'))
                                        minX = Number(getMinMaxXY('min', 'x'))
                                        minY = Number(getMinMaxXY('min', 'y'))
                                        centerX = (maxX+minX) / 2;
                                        centerY = (maxY+minY) / 2;
                                        initTmap();
                                    } else {
            ...
            function initTmap() {
                ...
                // 맵의 영역을 설정
                map.fitBounds(
                    new Tmapv2.LatLngBounds(new Tmapv2.LatLng(maxY, minX), new Tmapv2.LatLng(minY, maxX))
                )
                ...
                markerTouchAddListener(0, 'start')
                marker = new Tmapv2.Marker({
                ...
                markerTouchAddListener((routeDataList.length - 1), 'end')
            }
                    ...
                    for (let i = 1; i < routeDataList.length - 1; i++) {
                        if(routeDataList[i].locationId) {
                            if(i <= endStationLineSeq) {
                                ...
                                markerTouchAddListener(i, 'start')
                            }
                            if(i > endStationLineSeq) {
                                marker = new Tmapv2.Marker({
                                    position: new Tmapv2.LatLng( routeDataList[i].y, routeDataList[i].x ), //Marker의 중심좌표 설정.
            ...
            function markerTouchAddListener(index, direction) {
                // touchend
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(stationDataStatus === '0') {
                                ...
                                toReactNative={
                                    isError : true,
                                    data: stationData.response.msgHeader.resultMessage 
                                }
                                postMessageReactNative(JSON.stringify(toReactNative))
                            }
                        },
            ...
            function fitBounds(x, y) {
                let latlngSW = new Tmapv2.LatLng(y + 0.05, x - 0.05);
                let latlngNE = new Tmapv2.LatLng(y - 0.05, x + 0.05);
