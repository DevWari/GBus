...
// searchRoad() {
//     let serverURL = `https://topopen.tmap.co.kr/tmap/gg/transit/pathfind?appKey=` + Constants.APP_KEY + `&version=1&format=json&bizAppId=` + Constants.BIZ_APP_ID + `&reqCoordType=` + Constants.COORD_TYPE + `&resCoordType=` + Constants.COORD_TYPE + `&sX=` + this.state.startPOI.x + `&sY=` + this.state.startPOI.y + `&eX=` + this.state.endPOI.x + `&eY=` + this.state.endPOI.y;
//     showPageLoader(true);
//     return (
//         fetch(serverURL, { method: 'GET' })
//             .then((response) => {
//                 if (response.ok) { return response.json() }
//                 else { showPageLoader(false) }
//             }).then((json) => {
//                 // console.log(JSON.stringify(json, null, 4))
//                 if (Number(json?.PathFind.pointDistance) < 800) {
//                     Alert.alert('알림', '출발지와 도착지의 거리가 800m미만인 경우 대중교통 길찾기 결과를 제공하지 않습니다.',
//                         [{
//                             text: '확인'
//                         }]
//                     )
